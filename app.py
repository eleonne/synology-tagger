from src.model import get_images, save_tags, get_short_videos, get_long_videos, get_longest_videos
...
while(True):
    print('WAKING UP: ', os.path.dirname(os.path.realpath(__file__)), 'at', datetime.datetime.now())
    ...
    # reiterate the list from DB until all pics are tagged
    while len(list) > 0:
        print('PROCESSING LONG VIDEOS BATCH WITH', len(list), 'ITEMS')

        for row in tqdm(list): 
            start = time.time()
            frames = vd.extract_long_video_frames(source_folder + row.full_path)
            tags = vd.process_frames(frames['list'])
            if len(tags['list']) > 0:
                save_tags(tags['list'], row.unit_id)
            else:
    ...
    # reiterate the list from DB until all pics are tagged
    while len(list) > 0:
        print('PROCESSING LONGEST VIDEOS BATCH WITH', len(list), 'ITEMS')

        for row in tqdm(list): 
            start = time.time()
            frames = vd.extract_longest_video_frames(source_folder + row.full_path)
            tags = vd.process_frames(frames['list'])
            if len(tags['list']) > 0:
                save_tags(tags['list'], row.unit_id)
            else:
